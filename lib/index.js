...
class ActionSheet extends React.Component {
  ...
  _createButton (title, index) {
    const styles = this.styles
    const { buttonUnderlayColor, cancelButtonIndex, destructiveButtonIndex, tintColor, options } = this.props
    ...
    const topButtonStyle = Platform.OS === 'ios' && index === 0 ? styles.borderTopRadius : {};
    const lastButtonIndex = cancelButtonIndex ? options.length - 2 : options.length - 1;
    const bottomButtonStyle = Platform.OS === 'ios' && index === lastButtonIndex ? styles.borderBottomRadius : {};
    const boldStyle = Platform.OS === 'ios' && cancelButtonIndex === index ? styles.bold : {};
    ...
    return (
      <TouchableHighlight
        ...
        style={[buttonBoxStyle, topButtonStyle, bottomButtonStyle]}
        onPress={() => this.hide(index)}
      >
        {React.isValidElement(title) ? title : (
          <Text style={[styles.buttonText, boldStyle, {color: fontColor}]}>{title}</Text>
        )}
      </TouchableHighlight>
    )
  }
